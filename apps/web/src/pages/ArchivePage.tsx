      ...
      <div className="flex flex-col flex-1 min-h-0 min-w-0">
        <div className="shrink-0 px-4 md:px-0">
          <h1 className="text-lg font-semibold text-[var(--color-text)] mb-1">
            Archive{totalCount > 0 ? ` (${hasActiveFilters ? `${filteredCount}/${totalCount}` : totalCount})` : ''}
          </h1>
        ...
        <div className="overflow-y-auto flex-1 min-h-0 px-3 md:px-0">
          {totalCount === 0 && <EmptyState message="Nothing archived yet. Tasks you set aside will appear here." />}
          {totalCount > 0 && filtered.length === 0 && (
            <EmptyState message="No archived tasks match the current filters." />
          )}

...
- **Web Push client subscription — DONE.** Both sides are now wired. Server: `notifications.service.ts` (`web-push` + `setVapidDetails`) + `POST /api/push/subscribe`. Client: `hooks/usePushNotifications.ts` requests permission, calls `pushManager.subscribe({ applicationServerKey })` using `VITE_VAPID_PUBLIC_KEY`, and POSTs the subscription; toggled from **Settings → Notifications**. The SW (`public/sw.js`) already handles `push`/`notificationclick`. To activate: generate VAPID keys (`npx web-push generate-vapid-keys`), set `VAPID_PUBLIC_KEY`/`VAPID_PRIVATE_KEY`/`VAPID_SUBJECT` on the API and the **same** public key as `VITE_VAPID_PUBLIC_KEY` in the web build, then enable the Settings toggle and grant permission. Unset → server logs "VAPID keys not configured" and skips; the Settings toggle shows as unavailable.
